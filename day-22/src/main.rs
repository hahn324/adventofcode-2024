...
fn add_sequence(
    ...
    sequence_tracker: &mut HashSet<[isize; 4]>,
    banana_tracker: &mut HashMap<[isize; 4], isize>,
) {
    ...
    if !sequence_tracker.contains(price_sequence) {
        sequence_tracker.insert(*price_sequence);
        banana_tracker
            .entry(*price_sequence)
